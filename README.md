...
void MySystem::setup(Registry& reg) {
    ECS_REG_FUNC(reg, updateTransform);
    ECS_REG_FUNC(reg, updateCameraTransform);
    ECS_REG_FUNC(reg, update); // same registration for all functions
...
void MySystem::stop(Registry& reg) {
    ...
    ECS_UNREG_FUNC(reg, updateTransform);
...
// or you can use `world` or `observer`
```cpp
